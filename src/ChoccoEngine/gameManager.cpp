...
Window::Window() {
	...
	helloLmao = std::make_shared<Font> (renderer, "assets/ttf/sample.ttf", "HelloLmao", Color(255, 0, 255, 255), Vector2f(69, 420), "Haha lmao hello world lol.", 16);

	test = std::make_shared<Sprite> (renderer, "assets/img/TextureError.png", Vector2f(0, 0), Vector2f(1, 1), 0, "test");
}
...
void Window::drawSprite(std::shared_ptr<Sprite> sprite) {
	SDL_Rect dst;
...
void Window::drawFont(std::shared_ptr<Font> font) {
	SDL_Rect dst;
...
void Window::drawAllSprites(std::shared_ptr<SpriteGroup> group) {
